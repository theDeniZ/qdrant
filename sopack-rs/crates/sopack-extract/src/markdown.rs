//! Markdown → [`sopack_book::Book`]. Port of `sopack/extract/markdown.py`.
//!
//! A heading (`#` … `######`) starts a new `page` (so a book chaptered with
//! Markdown headings gets one page per chapter); paragraphs between
//! headings — split on blank lines — are numbered sequentially within that
//! page. Long paragraphs are chunked the same way as the EPUB extractor
//! ([`crate::chunk`]); short/damaged/junk paragraphs are dropped and
//! recorded in `stats["dropped_detail"]` (R8).
//!
//! Metadata is never guessed from the filename (rule #9) — it comes from
//! [`crate::ExtractOptions`]; what is not given is left `None` for hand
//! entry in the reviewed book.json.
...
static HEADING_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^#{1,6}\s+").expect("valid HEADING_RE"));
static FENCE_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^```").expect("valid FENCE_RE"));
static SPACE_RUN_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"[ \t]+").expect("valid space run"));

/// Split *text* on every non-overlapping match of *re*, like Python's
/// `re.split` (with no capture groups in the pattern, which is all this
/// crate's splitter patterns use).
...
pub fn extract(
    ...
    for raw_para in regex_split(&BLANK_RE, &raw) {
        ...
        if text.is_empty() || FENCE_RE.is_match(text).unwrap_or(false) {
            continue;
        }
        blocks_in += 1;

        if HEADING_RE.is_match(text).unwrap_or(false) {
            page += 1;
            para_in_page = 0;
            let stripped = regex_replace_all(&HEADING_RE, text, "").trim().to_string();
            if !stripped.is_empty() {
                dropped_detail.push(dropped_entry(
                    &format!("{page}.0"),
                    "heading",
                    Some(&char_truncate(&stripped, 80)),
                ));
            }
            continue;
        }

        let text = regex_replace_all(&SPACE_RUN_RE, text, " ");
        para_in_page += 1;
        let para_key = format!("{page}.{para_in_page}");
