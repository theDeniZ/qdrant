"""Markdown → :class:`~sopack.book.Book`.

A heading (``#`` … ``######``) starts a new ``page`` (so a book chaptered
with Markdown headings gets one page per chapter); paragraphs between
headings — split on blank lines — are numbered sequentially within that page.
Long paragraphs are chunked the same way as the EPUB extractor
(``extract.chunk``); short/damaged/junk paragraphs are dropped and recorded
in ``stats["dropped_detail"]`` (R8).
...
def extract(
    ...
    *,
    ...
    blocks_in = 0
    page, para_in_page = 1, 0

    for raw_para in _BLANK_RE.split(raw):
        text = raw_para.strip()
        if not text or _FENCE_RE.match(text):
            continue
        blocks_in += 1
        ...
        if _HEADING_RE.match(text):
            page += 1
            para_in_page = 0
            heading_text = _HEADING_RE.sub('', text).strip()
            if heading_text:
                dropped_detail.append({"para_key": f"{page}.0", "reason": "heading",
                                        "text": heading_text[:80]})
            continue

        text = re.sub(r'[ \t]+', ' ', text)
        para_in_page += 1
        para_key = f"{page}.{para_in_page}"
        ...
        for j, piece in enumerate(pieces):
            out_blocks.append(Block(
                para_key=para_key, page=page, para=para_in_page,
